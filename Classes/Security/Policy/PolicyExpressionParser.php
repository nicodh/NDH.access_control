	...
	protected function parseSingleEntityResource($resourceName, array $entityResourcesTree) {
		...
		for ($i = 0; $i < count($expressionParts); $i += 2) {
			...
			if (preg_match('/\s(==|!=|<=|>=|<|>|in|contains|matches)\s/', $expressionParts[$i]) > 0) {
				...
				if (!isset($entityResourcesTree[$expressionParts[$i]])) throw new \NDH\AccessControl\Security\Exception\NoEntryInPolicyException('Entity resource "' . $expressionParts[$i] . '" not found in policy.', 1267722067);
